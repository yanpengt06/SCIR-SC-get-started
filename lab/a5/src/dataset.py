...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        doc = self.data[idx]
        doc_len = len(doc)
        upper = doc_len if doc_len < self.block_size * 7 / 8  else self.block_size * 7 / 8
        length = random.randint(4, upper)
        end = doc_len - length
        trunc_start = random.randint(0, end)        # 在可能的范围内随机裁剪
        truncated = doc[trunc_start: trunc_start + length]
        trunc_len = len(truncated)
        # 平均四分之一长度，mask在八分之一与八分之三之间取值
        masked_len = random.randint(round(1 / 8 * trunc_len), round(3 / 8 * trunc_len))
        masked_end = trunc_len - masked_len
        masked_start = random.randint(0, masked_end)        # 在可能的范围内随机遮罩
        masked =  truncated[masked_start : masked_start + masked_len]
        prefix = truncated[0: masked_start]
        suffix = truncated[masked_start + masked_len : ]
        """
        [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] [pads]
        """
        result_str = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked + \
                     self.PAD_CHAR * (self.block_size - trunc_len - 2)
        input = result_str[:-1]
        output = result_str[1:]
        input = torch.LongTensor([self.stoi[x] for x in input])
        output = torch.LongTensor([self.stoi[x] for x in output])
        return input, output
"""
...
if __name__ == '__main__':
    ...
    if args.dataset_type == 'namedata':
        ...
        corruption_dataset = CharCorruptionDataset(open('wiki.txt', encoding="UTF-8").read(), 128)
        for _, example in zip(range(16), corruption_dataset):
            x, y = example
