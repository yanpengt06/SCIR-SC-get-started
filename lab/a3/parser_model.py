...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden = torch.nn.Linear(self.embed_size * self.n_features, hidden_size, bias=True)
        torch.nn.init.xavier_uniform_(self.embed_to_hidden.weight,gain=1)
        self.dropout = torch.nn.Dropout(dropout_prob)
        self.hidden_to_logits = torch.nn.Linear(hidden_size, n_classes)
